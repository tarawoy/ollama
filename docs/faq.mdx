...
- `OLLAMA_KV_CACHE_TYPE_K` - The quantization type for the K cache. Overrides `OLLAMA_KV_CACHE_TYPE` when set.
- `OLLAMA_KV_CACHE_TYPE_V` - The quantization type for the V cache. Overrides `OLLAMA_KV_CACHE_TYPE` when set.
...
If `OLLAMA_KV_CACHE_TYPE_K` and `OLLAMA_KV_CACHE_TYPE_V` are not set, Ollama falls back to `OLLAMA_KV_CACHE_TYPE` for both.

The currently available K/V cache quantization types and aliases are:

- `f16` - high precision and memory usage (default).
- `q8_0` - 8-bit quantization, uses approximately 1/2 the memory of `f16` with a very small loss in precision, this usually has no noticeable impact on the model's quality (recommended if not using f16).
- `q4_0` - 4-bit quantization, uses approximately 1/4 the memory of `f16` with a small-medium loss in precision that may be more noticeable at higher context sizes.
- `tq1_0` (alias: `turbo2`) - ternary cache quantization with higher compression than `q4_0`.
- `tq2_0` (aliases: `turbo3`, `turboquant`, `turboquant google`) - ternary cache quantization tuned for stronger quality/compression balance.
- `turbo4` - alias of `q4_0`.

Example asymmetric configuration:
